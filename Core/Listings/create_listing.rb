...
def CreateListing(isAdmin, key, user, userId, landlord, landlordId, price, address, unit, bedrooms, bathrooms, animals, laundry, parking, airConditioning, type, notes, start, latitude, longitude, university, tags, pictures)
    ...
    begin
        mongoSession.with(safe: true) do |session|
            ...
            if currListings.count > 0 and not userId.nil?
                ...
                document = session[:listings].find(retQueryObj).select(_id: 1, Username: 1, Price: 1, Address: 1, Unit: 1, Bedrooms: 1, Bathrooms: 1, HasAnimals: 1, HasLaundry: 1, HasParking: 1, HasAirConditioning: 1, Type: 1, Notes: 1, Start: 1, WorldCoordinates: 1, Landlord: 1, University: 1, Tags: 1, Pictures: 1, Thumbnails: 1).one
            end
        end
...
begin
    ...
    result = CreateListing(isAdmin, key, user, userId, landlord, landlordId, data["Rent"], data["Address"], data["Unit"], data["Bedrooms"], data["Bathrooms"], data["Animals"], data["Laundry"], data["Parking"], data["AirConditioning"], data["Type"], data["Notes"], data["Start"], data["Latitude"], data["Longitude"], data["University"], data["Tags"], data["Pictures"])
